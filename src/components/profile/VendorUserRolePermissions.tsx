    ...
    const permissions = (entity: string, map: any, userRole: UserRole) => {
        ...
        return (
            <div className="flex mb-2">
                ...
                <select className="outline-none rounded-md border border-custom-theme cursor-pointer w-20 md:w-full bg-white" defaultValue={vendorUserRolePermissions ? vendorUserRolePermissions.permissions?.[userRole]?.[entity] : (rolePermissions as any).permissions?.[userRole]?.[entity]} onChange={(e) => { handleChange(entity, e.target.value as RolePermissions, userRole) }}>
