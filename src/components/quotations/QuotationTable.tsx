                    ...
                    <tbody>
                        {filteredQuotations.map((quotation: Quotation, index:number) => (
                            <tr key={quotation.quotationId} className="border-b border-black">
                                ...
                                <button className={'bg-custom-red hover:bg-hover-red px-5 py-2 text-white rounded-md'} onClick={() => router.push(`/quotations/${quotation.quotationId}`)}>{quotation.status === QuotationStatus.ACCEPTED ? "View" : "Edit"}</button>
                                </td>
                                {quotation.status === QuotationStatus.ACCEPTED && <td className="p-2 text-center align-middle">
                                    <button className='bg-custom-red hover:bg-hover-red px-5 py-2 text-white rounded-md flex gap-2' onClick={() => router.push(`/orders/create/${quotation.quotationId}`)}>
                                        <ShoppingCartIcon />
